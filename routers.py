...
async def studies_analyze(study_id: int, input_data: StudyAnalyzeInput = None, user: dict = Depends(require_user)):
    ...
    try:
        ...
        if template:
            ...
            analysis_result = await analyze_template_form(image_urls, template_dicts)
            ...
            analysis_result = await analyze_xray_image(image_urls, study["studyType"], user_query=user_query)
            analysis_result_str = analysis_result
